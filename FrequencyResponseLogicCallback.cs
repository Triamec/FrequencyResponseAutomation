...
namespace Triamec.Tam.FrequencyResponse.NUnit {
	...
	internal class FrequencyResponseLogicCallback : IDisposable {
		...
		public FrequencyResponseLogicCallback(IFrequencyResponseLogic logic, AutoResetEvent signal, BaseTestFixture testFixture,
			CultureInfo formatProvider, ILog log) {
			...
			string testDir = Path.Combine(testFixture.TestDataDirectory, "Frequency Response");
		...
		void OnGetFrequencyResponseResultCompletedEvent(object sender, FrequencyResponseResultCompletedEventArgs args) {
			...
			if (result != null && _log.IsInfoEnabled) {
